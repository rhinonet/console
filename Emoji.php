...
//var_dump(++$i,$matches[0][$i-1])));
            //$key = '_$'.$i++.'_';
            //$matches[$key] = $match[0];
            //return $key;

++$i;
	  return  urlencode($matches[0][$i-1]);
        });
...
$c = preg_replace_callback('/(%[0-9A-F][0-9A-F]){4}/', function($match)use(&$i, &$matches1){
